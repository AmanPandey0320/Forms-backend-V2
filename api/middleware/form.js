...
const IS_FORM_VALID = async (req,res,next) => {
    ...
    try{
        ...
        pool.query(sql,[form_id],(err,result)=>{
            if(err){
                console.log(err);
                status = `operation failed with an error : ${JSON.stringify(err)} for user ${user_id}`;
                logs.add_log(ip,endpoint.info.status);
                return res.sendStatus(500);
            }
        ...
        console.log(error);
        status = `operation failed with an error : ${JSON.stringify(error)} for the user ${user_id}`
        logs.add_log(ip,endpoint,info,status);
        res.sendStatus(500);
