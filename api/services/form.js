...
class FormService {
  ...
  GET_ALL_SEC_SQL = `SELECT asec.id,asec.title,asec.description,asec.order,asec.theme,asec.last_edited,aq.when FROM akp_section as asec WHERE asec.fid = ? AND asec.active = true`;
  GET_ALL_QUE_SQL = `SELECT aq.id,aq.title,aq.description,aq.order,aq.type,aq.when,aq.active,aq.last_edited,aq.required,aq.marks FROM akp_question as aq WHERE aq.fid = ? AND aq.active = true`;
  /**
